...
class LegerLoader:
    ...
    def update_mart(self):
        with self.pg_dest.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    WITH rate AS(
                    SELECT dd.courier_id, dd.order_id, AVG(fd.rate) over(PARTITION BY dd.courier_id) avg_rate
                    FROM dds.fct_courier_deliveries fd
                    ...
                    tmp AS (SELECT dd.courier_id,
                        ...
                        SUM(CASE			    
                                WHEN rate.avg_rate < 4 THEN greatest(fps.total_sum * 0.05, 100)
                                WHEN rate.avg_rate >= 4 AND rate.avg_rate < 4.5 THEN greatest(fps.total_sum * 0.07, 150)
                                WHEN rate.avg_rate >= 4.5 AND rate.avg_rate < 4.9 THEN greatest(fps.total_sum * 0.07, 175)
                                WHEN rate.avg_rate >= 4.9 THEN greatest (fps.total_sum, 200)
                            END) AS courier_order_sum,
                        SUM(fd.tip_sum) courier_tips_sum
                    FROM dds.fct_courier_deliveries fd
                    JOIN dds.dm_deliveries dd on fd.delivery_id = dd.id
                    JOIN dds.fct_product_sales fps on fps.order_id = dd.order_id
